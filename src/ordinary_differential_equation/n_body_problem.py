...
# 粒子の加速度を計算する関数
def calculate_acceleration(masses: np.ndarray, positions: np.ndarray) -> np.ndarray:
    """
...
def runge_kutta_step(del_t: float, masses: np.ndarray, current_state: np.ndarray):
    """
...
if N_body > len(colors):
    print("Warning: N_body is greater than available colors. Colors will be repeated.")
...
def update(i):
    ...
    ax.set_title(f"N-Body Gravitational Problem (N={N_body}) - Time: {t_list[data_index]:.2f}")
